...
const TARGET_AUDIENCES: string[] = ['Particuliers', 'Petites entreprises', 'Grandes entreprises', 'Gouvernement'];
...
export class PublishApp implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (idParam) {
      ...
      this.appService.getById(this.editId).subscribe({
        ...
        error: () => {
          this.errorMessage.set(this.ts.t('publish.errorLoad'));
          this.loadingApp.set(false);
        },
      });
    }
  ...
  togglePlatform(platform: SupportedPlatform): void {
    ...
    idx >= 0 ? this.supportedPlatforms.splice(idx, 1) : this.supportedPlatforms.push(platform);
  }

  toggleSector(sector: string): void {
    const idx = this.selectedBusinessSectors.indexOf(sector);
    idx >= 0 ? this.selectedBusinessSectors.splice(idx, 1) : this.selectedBusinessSectors.push(sector);
  }

  toggleSupportOption(option: string): void {
    const idx = this.selectedSupportOptions.indexOf(option);
    idx >= 0 ? this.selectedSupportOptions.splice(idx, 1) : this.selectedSupportOptions.push(option);
  }

  onIconSelected(event: Event): void {
    const file = (event.target as HTMLInputElement).files?.[0];
    if (!file) return;
    const err = this.uploadService.validateFile(file);
    if (err) { this.stepError.set(err); return; }
    this.uploadingIcon.set(true);
    this.uploadService.uploadSingle(file).subscribe({
      next: (res) => { this.iconUrl = res.data.url; this.uploadingIcon.set(false); },
      error: () => { this.stepError.set(this.ts.t('publish.errorIcon')); this.uploadingIcon.set(false); },
    });
  }
  ...
  onScreenshotsSelected(event: Event): void {
    ...
    this.uploadService.uploadMultiple(files).subscribe({
      next: (res) => { this.screenshots = [...this.screenshots, ...res.data.map((f) => f.url)]; this.uploadingScreenshots.set(false); },
      error: () => { this.stepError.set(this.ts.t('publish.errorScreenshots')); this.uploadingScreenshots.set(false); },
    });
  }

  removeScreenshot(url: string): void { this.screenshots = this.screenshots.filter((s) => s !== url); }

  goToStep(step: number): void {
    if (step < this.currentStep()) { this.currentStep.set(step); this.stepError.set(null); }
  }
  ...
  previous(): void { this.stepError.set(null); this.currentStep.set(Math.max(1, this.currentStep() - 1)); }

  private validateStep(step: number): boolean {
    const required = this.ts.t('publish.errorRequired');
    if (step === 1 && (!this.appName || !this.description || !this.categoryId || !this.targetAudience)) {
  ...
  submit(): void {
    ...
    const payload: AppCreatePayload = {
      app_name: this.appName, tagline: this.tagline || undefined,
      description: this.description, detailed_description: this.description,
      category_id: this.categoryId!, subcategory: this.subcategory || undefined,
      target_audience: this.targetAudience || undefined, icon_url: this.iconUrl || undefined,
      ...
      business_value: this.businessValue || undefined, key_features: this.splitLines(this.keyFeaturesInput),
